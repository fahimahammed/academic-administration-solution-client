...
const EditFaculty = ({ id, base }: EditStudentProps) => {
	...
	const defaultValue = {
		...
		gender: data?.gender || '',
		academicDepartmentId: data?.academicDepartment?.id,
		academicFacultyId: data?.academicFaculty?.id,
	};
		...
		<>
			<BreadCrumbsComp
				items={[
					...
					{ label: 'edit', link: '' },
					{ label: data?.userId, link: `/${base}/faculty/edit/${data?.userId}` },
				]}
			/>
			<ActionBar title={`edit faculty - ${data?.userId}`}></ActionBar>
			<Form onSubmit={facultyOnSubmit} defaultValues={defaultValue}>
				...
				<div style={{ border: '1px solid #d9d9d9', borderRadius: '5px', padding: '15px', marginBottom: '10px' }}>
					...
					<Row gutter={{ xs: 24, xl: 8, lg: 8, md: 24 }}>
						...
						<Col span={6} style={{ margin: '10px 0' }}>
							<AcademicDepartmentField name="academicDepartmentId" label="academic department" />
						</Col>
