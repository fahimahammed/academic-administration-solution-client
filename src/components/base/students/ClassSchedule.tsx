		...
		<>
			{data?.map((schedule: IOfferedCourseSchedule, index: number) => {
				return (
					<div
						key={index}
						style={{ border: '1px solid #d9d9d9', borderRadius: '5px', marginBottom: '5px', padding: '10px' }}
					>
						<div>
							<b style={{ textTransform: 'capitalize' }}>faculty name</b>
							<Link href={`/student/faculty/${schedule.faculty.id}`} style={{ marginLeft: '20px' }}>
						...
						<div>
							...
							<span style={{ marginLeft: '20px' }}>{schedule.room.building.title}</span>
						</div>
						<div>
							<b style={{ textTransform: 'capitalize' }}>room no.</b>
							<span style={{ marginLeft: '20px' }}>{schedule.room.roomNumber}</span>
						</div>
						<div>
